"""test_singletontable.py
Tests for the singletontable module.
"""
...
from sqlalchemyobjects.tables import BaseSingletonTableSchema, SingletonTableManifestation
from sqlalchemyobjects.testsuite import SingletonTableManifestationTestSuite, SingletonTableSchemaTestSuite
...
class ConcreteTableSchema(BaseSingletonTableSchema, ConcreteDatabaseSchema):
    """A concrete Table Schema for testing."""
    __tablename__ = "concrete_table"
    name: Mapped[str | None] = mapped_column(String, nullable=True)


class ConcreteTableManifestation(SingletonTableManifestation):
    """A concrete Table Manifestation for testing."""
    table_schema: type[ConcreteTableSchema] = ConcreteTableSchema


class TestBaseSingletonTableSchema(SingletonTableSchemaTestSuite):
    """Tests the BaseSingletonTableSchema class."""
    ...
    def test_set_item(self) -> None:
        ...
        try:
            ...
            with Session(db_engine) as session:
                ...
                cls.set_item(session, **update_data)
                session.commit()
    ...
    async def test_set_item_async(self) -> None:
        ...
        try:
            ...
            async with AsyncSession(db_engine) as session:
                ...
                await cls.set_item_async(session, **update_data)
                await session.commit()
    ...
    def test_set_item_new(self) -> None:
        """Tests the set_item method when the table is empty."""
        item = self.generate_item()
        db_engine = self.create_engine()
        try:
            cls = self.UnitTestClass
            with Session(db_engine) as session:
                cls.set_item(session, **item)
                session.commit()
    ...
    def test_set_item_new_with_begin(self) -> None:
        """Tests the set_item method when the table is empty and begin=True."""
        item = self.generate_item()
        db_engine = self.create_engine()
        try:
            cls = self.UnitTestClass
            with Session(db_engine) as session:
                cls.set_item(session, begin=True, **item)
                # Commit is handled by begin=True within set_item?
                # Wait, set_item calls insert. insert(begin=False) if set_item(begin=True) calls it?
                # In set_item: if begin: with session.begin(): ... cls.insert(..., begin=False).
                # So session.begin() handles commit.
                # But the session created here `with Session(db_engine)` is not autocommit.
                # `session.begin()` creates a subtransaction or transaction.
                # If `session` is already in transaction? `Session(db_engine)` starts clean.
                # `session.begin()` will fail if session is already active? No, nesting?
    ...
    @pytest.mark.asyncio
    async def test_set_item_async_new(self) -> None:
        """Tests the set_item_async method when the table is empty."""
        item = self.generate_item()
        db_engine = await self.create_async_engine()
        try:
            cls = self.UnitTestClass
            async with AsyncSession(db_engine) as session:
                await cls.set_item_async(session, **item)
                await session.commit()
    ...
    @pytest.mark.asyncio
    async def test_set_item_async_new_with_begin(self) -> None:
        """Tests the set_item_async method when the table is empty and begin=True."""
        item = self.generate_item()
        db_engine = await self.create_async_engine()
        try:
            cls = self.UnitTestClass
            async with AsyncSession(db_engine) as session:
                await cls.set_item_async(session, begin=True, **item)
                # async session.begin() commits on exit.
...
class TestSingletonTableManifestation(SingletonTableManifestationTestSuite):
    """Tests the SingletonTableManifestation class."""
    ...
    def test_upsert(
        self,
        test_object: SingletonTableManifestation,
        use_session: bool,
        entry: dict[str, Any],
        begin: bool,
        update: bool,
